...
def train(args):
  ...
  for i in range(num_epochs):
    ...
    for im, target, _ in tqdm(train_dataset):
    ...
    loss_output = ''
    loss_output += 'RPN Classification Loss: {:.4f}\n'.format(np.mean(rpn_classification_losses))
    loss_output += 'RPN Localization Loss: {:.4f}\n'.format(np.mean(rpn_localization_losses))
    loss_output += 'FRCNN Classification Loss: {:.4f}\n'.format(np.mean(frcnn_classification_losses))
    loss_output += 'FRCNN Localization Loss: {:.4f}\n'.format(np.mean(frcnn_localization_losses))
    print(loss_output)
    scheduler.step()
  print('Done Training')
